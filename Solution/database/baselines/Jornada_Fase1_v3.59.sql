...
-- v3.59: canonicalização brasileira V2. Dados nacionais sem +55 (10/11 dígitos) convergem ao mesmo E.164 lógico.
UPDATE silver.pessoa_atributo_observacao
SET atributo_instancia_chave='55'+atributo_instancia_chave
WHERE atributo_codigo='TELEFONE_CONTATO' AND LEN(atributo_instancia_chave) IN(10,11)
  AND atributo_instancia_chave NOT LIKE '%[^0-9]%';
GO
...
-- v3.59: migra chaves nacionais legadas para o mesmo identificador canônico com DDI 55.
UPDATE gold.pessoa_atributo
SET atributo_instancia_chave='55'+atributo_instancia_chave,atualizado_em=SYSDATETIMEOFFSET()
WHERE atributo_codigo='TELEFONE_CONTATO' AND LEN(atributo_instancia_chave) IN(10,11)
  AND atributo_instancia_chave NOT LIKE '%[^0-9]%';
-- Se representações com/sem DDI coexistiam como correntes, conserva a evidência de maior precedência e encerra as duplicatas lógicas.
;WITH d AS(
...
BEGIN
 ...
 -- FUSAO_HISTORICA é exaustiva por UUID absorvido. Não permite sucesso parcial que apenas reclassifique parte dos vínculos.
 IF @motivo='FUSAO_HISTORICA' AND EXISTS(
   SELECT 1
   ...
   THROW 51119,'Fusão histórica incompleta: o UUID a absorver possui observações RESOLVIDAS não incluídas integralmente no mesmo destino.',1;
 ...
 BEGIN
   INSERT @fundidos(origem,dest)
   SELECT c.origem,c.dest FROM @convergencia c
   WHERE c.origem<>c.dest
     AND NOT EXISTS(SELECT 1 FROM identidade.v_vinculo_corrente v WHERE v.pessoa_uuid=c.origem AND v.status='RESOLVIDO');
   IF EXISTS(SELECT 1 FROM @fundidos f WHERE identidade.fn_pessoa_uuid_canonico(f.dest) IS NULL OR identidade.fn_pessoa_uuid_canonico(f.dest)=f.origem)
 ...
 -- CPF divergente não é automaticamente acumulado na Pessoa canônica: exige resolução governada separada.
 IF EXISTS(
   SELECT 1 FROM identidade.identity_map src JOIN @fundidos f ON f.origem=src.pessoa_uuid
