...
IF OBJECT_ID('ref.tipo_registro_versao','U') IS NULL CREATE TABLE ref.tipo_registro_versao(
 ...
 origina_endereco_casa_abrigo_sigilosa BIT NOT NULL DEFAULT(0),
...
GO
-- v3.57 normativa: endereço de casa-abrigo-sigilosa só pode nascer de Tipo de Serviço explicitamente habilitado.
IF COL_LENGTH('ref.tipo_registro_versao','origina_endereco_casa_abrigo_sigilosa') IS NULL
 ALTER TABLE ref.tipo_registro_versao ADD origina_endereco_casa_abrigo_sigilosa BIT NOT NULL CONSTRAINT DF_trv_origem_casa_abrigo_sigilosa DEFAULT(0);
GO
...
BEGIN
 ...
 IF EXISTS(SELECT 1 FROM inserted i JOIN ref.tipo_registro tr ON tr.tipo_registro_id=i.tipo_registro_id
           WHERE i.origina_endereco_casa_abrigo_sigilosa=1 AND tr.natureza<>'SERVICO')
   THROW 51076,'ENDERECO_CASA_ABRIGO_SIGILOSA só pode ser originado por Tipo de Serviço explicitamente habilitado.',1;
END;
GO
...
IF OBJECT_ID('ref.atributo_transversal','U') IS NULL CREATE TABLE ref.atributo_transversal(
 ...
 cardinalidade NVARCHAR(10) NOT NULL CONSTRAINT df_atributo_transversal_cardinalidade DEFAULT('SINGLE'),
 chave_instancia_codigo NVARCHAR(80) NOT NULL CONSTRAINT df_atributo_transversal_chave DEFAULT('UNICA_V1'),
 descricao NVARCHAR(1000) NULL,
 ativo BIT NOT NULL DEFAULT(1),
 criado_em DATETIMEOFFSET(7) NOT NULL DEFAULT(SYSDATETIMEOFFSET()),
 CONSTRAINT ck_atributo_transversal_cardinalidade CHECK(cardinalidade IN('SINGLE','MULTI')),
 CONSTRAINT ck_atributo_transversal_chave CHECK((cardinalidade='SINGLE' AND chave_instancia_codigo='UNICA_V1') OR (cardinalidade='MULTI' AND chave_instancia_codigo IN('TELEFONE_BR_CANONICO_V2','EMAIL_CANONICO_V2'))));
GO
IF COL_LENGTH('ref.atributo_transversal','cardinalidade') IS NULL
...
IF EXISTS(SELECT 1 FROM sys.check_constraints WHERE parent_object_id=OBJECT_ID('ref.atributo_transversal') AND name='ck_atributo_transversal_cardinalidade') ALTER TABLE ref.atributo_transversal DROP CONSTRAINT ck_atributo_transversal_cardinalidade;
ALTER TABLE ref.atributo_transversal WITH CHECK ADD CONSTRAINT ck_atributo_transversal_cardinalidade CHECK(cardinalidade IN('SINGLE','MULTI'));
IF EXISTS(SELECT 1 FROM sys.check_constraints WHERE parent_object_id=OBJECT_ID('ref.atributo_transversal') AND name='ck_atributo_transversal_chave') ALTER TABLE ref.atributo_transversal DROP CONSTRAINT ck_atributo_transversal_chave;
ALTER TABLE ref.atributo_transversal WITH CHECK ADD CONSTRAINT ck_atributo_transversal_chave CHECK((cardinalidade='SINGLE' AND chave_instancia_codigo='UNICA_V1') OR (cardinalidade='MULTI' AND chave_instancia_codigo IN('TELEFONE_BR_CANONICO_V2','EMAIL_CANONICO_V2')));
GO
-- v3.60: equivalente SQL da regra TELEFONE_BR_CANONICO_V2 para upgrade/reconciliação de chaves legadas.
-- Retorna NULL quando o valor não satisfaz o contrato V2; a ingestão runtime continua validada no Processor.
CREATE OR ALTER FUNCTION ref.fn_telefone_br_canonico_v2(@valor NVARCHAR(2000))
RETURNS NVARCHAR(32)
AS
BEGIN
 DECLARE @v NVARCHAR(2000)=COALESCE(@valor,N''),@digits NVARCHAR(2000)=N'',@i INT=1,@c NCHAR(1),@explicit BIT=0,@n INT;
 -- Contrato V2: o envelope ignora somente SPACE/TAB/CR/LF/NBSP nas extremidades, igual ao Processor.
 WHILE DATALENGTH(@v)>0 AND UNICODE(LEFT(@v,1)) IN(9,10,13,32,160) SET @v=SUBSTRING(@v,2,2000);
 SET @n=DATALENGTH(@v)/2;
 WHILE @n>0 AND UNICODE(SUBSTRING(@v,@n,1)) IN(9,10,13,32,160)
 BEGIN
   SET @v=LEFT(@v,@n-1);
   SET @n=DATALENGTH(@v)/2;
 END;
 IF LEFT(@v,1)=N'+' OR LEFT(@v,2)=N'00' SET @explicit=1;
 WHILE @i<=LEN(@v)
 BEGIN
   SET @c=SUBSTRING(@v,@i,1);
   IF UNICODE(@c) BETWEEN 48 AND 57 SET @digits=@digits+@c;
   SET @i+=1;
 END;
 IF @explicit=1
 BEGIN
   IF LEFT(@digits,2)=N'00' SET @digits=SUBSTRING(@digits,3,30);
   IF LEN(@digits) BETWEEN 8 AND 15 RETURN @digits;
   RETURN NULL;
 END;
 IF LEN(@digits) IN(10,11) RETURN N'55'+@digits;
 IF LEN(@digits) IN(12,13) AND LEFT(@digits,2)=N'55' RETURN @digits;
 RETURN NULL;
END;
GO

-- v3.68: função legada recuperada da linha v3.66 apenas para diagnóstico/migração histórica.
-- EMAIL_CASEFOLD_V1 NÃO é a regra vigente: LOWER depende de collation e não reproduz a normalização Unicode do .NET.
CREATE OR ALTER FUNCTION ref.fn_email_casefold_v1(@valor NVARCHAR(2000))
RETURNS NVARCHAR(512)
AS
BEGIN
 DECLARE @v NVARCHAR(2000)=COALESCE(@valor,N''),@n INT,@at INT;
 WHILE DATALENGTH(@v)>0 AND UNICODE(LEFT(@v,1)) IN(9,10,13,32,160) SET @v=SUBSTRING(@v,2,2000);
 SET @n=DATALENGTH(@v)/2;
 WHILE @n>0 AND UNICODE(SUBSTRING(@v,@n,1)) IN(9,10,13,32,160)
 BEGIN
   ...
   SET @n=DATALENGTH(@v)/2;
 END;
 SET @at=CHARINDEX(N'@',@v);
 IF @at<=1 OR @at>=DATALENGTH(@v)/2 OR CHARINDEX(N'@',@v,@at+1)>0 RETURN NULL;
 RETURN LOWER(@v);
END;
GO

-- v3.68 / Base v3.62: EMAIL_CANONICO_V2 possui semântica deliberadamente reproduzível em SQL e C#:
-- trim SOMENTE SPACE/TAB/CR/LF/NBSP nas extremidades; exatamente um @ com local/domínio não vazios;
-- lowercase apenas para ASCII A-Z, preservando todos os demais codepoints e sem NFC/NFD implícito.
CREATE OR ALTER FUNCTION ref.fn_email_canonico_v2(@valor NVARCHAR(2000))
RETURNS NVARCHAR(512)
AS
BEGIN
 DECLARE @v NVARCHAR(2000)=COALESCE(@valor,N''),@out NVARCHAR(2000)=N'',@n INT,@i INT=1,@c NCHAR(1),@u INT,@at INT;
 WHILE DATALENGTH(@v)>0 AND UNICODE(LEFT(@v,1)) IN(9,10,13,32,160) SET @v=SUBSTRING(@v,2,2000);
 SET @n=DATALENGTH(@v)/2;
 WHILE @n>0 AND UNICODE(SUBSTRING(@v,@n,1)) IN(9,10,13,32,160)
 BEGIN
 ...
 SET @n=DATALENGTH(@v)/2;
 SET @at=CHARINDEX(N'@',@v);
 IF @at<=1 OR @at>=@n OR CHARINDEX(N'@',@v,@at+1)>0 RETURN NULL;
 WHILE @i<=@n
 BEGIN
   SET @c=SUBSTRING(@v,@i,1); SET @u=UNICODE(@c);
   IF @u BETWEEN 65 AND 90 SET @out=@out+NCHAR(@u+32); ELSE SET @out=@out+@c;
   SET @i+=1;
 END;
 IF DATALENGTH(@out)/2>512 RETURN NULL;
 RETURN @out;
END;
GO
-- Código reservado da plataforma. Não representa 'proteção da Pessoa': representa somente o endereço
-- declarado pelo próprio serviço de casa-abrigo-sigilosa. Não é usado como Referência Territorial.
IF NOT EXISTS(SELECT 1 FROM ref.atributo_transversal WHERE atributo_codigo='ENDERECO_CASA_ABRIGO_SIGILOSA')
 INSERT ref.atributo_transversal(atributo_codigo,nome,formato_codigo,regra_temporal_codigo,descricao)
 VALUES('ENDERECO_CASA_ABRIGO_SIGILOSA',N'Endereço de casa-abrigo-sigilosa','ENDERECO_CANONICO_V1','REFERENCIA_EVIDENCIA_OU_VERIFICADO_EM_V1',
        N'Endereço sigiloso declarado exclusivamente pelo próprio Tipo de Serviço de casa-abrigo explicitamente habilitado. Não é inferido pela Jornada, não qualifica a Pessoa como protegida e não alimenta Referência Territorial.');
GO
...
BEGIN
 ...
 -- Sigilo estrutural: o endereço de casa-abrigo-sigilosa jamais é projetado para Gestor diferente do responsável.
 -- Não depende de cadastro manual de restrição e não cria uma categoria de 'Pessoa protegida'.
 IF @alvo_tipo='ATRIBUTO_PESSOA' AND @alvo_codigo='ENDERECO_CASA_ABRIGO_SIGILOSA'
    AND @gestor_responsavel_id<>@gestor_consumidor_id RETURN 0;
 IF EXISTS(
   SELECT 1
...
GO
-- v3.88: objeto_chave permanece NVARCHAR(1024), mas não integra mais a chave do índice.
-- A chave Bronze é content-addressed por SHA-256; indexar payload_sha256 elimina o limite
-- potencial de 2048 bytes do índice antigo e preserva busca segura com confirmação por objeto_chave.
IF NOT EXISTS(SELECT 1 FROM sys.indexes WHERE object_id=OBJECT_ID('bronze.entrega_arquivo') AND name='IX_bronze_entrega_arquivo_payload_sha256')
    CREATE INDEX IX_bronze_entrega_arquivo_payload_sha256 ON bronze.entrega_arquivo(payload_sha256) INCLUDE(entrega_id,objeto_chave,tamanho_bytes);
IF EXISTS(SELECT 1 FROM sys.indexes WHERE object_id=OBJECT_ID('bronze.entrega_arquivo') AND name='IX_bronze_entrega_arquivo_objeto_chave')
...
IF OBJECT_ID('silver.pessoa_atributo_observacao','U') IS NULL CREATE TABLE silver.pessoa_atributo_observacao(
 ...
 atributo_instancia_chave NVARCHAR(512) NOT NULL CONSTRAINT df_silver_pat_instancia DEFAULT('#'),
...
GO
IF COL_LENGTH('silver.pessoa_atributo_observacao','atributo_instancia_chave') IS NULL
 ALTER TABLE silver.pessoa_atributo_observacao ADD atributo_instancia_chave NVARCHAR(512) NULL;
-- Fail-closed ANTES do backfill/ALTER NOT NULL para não mascarar legado inválido com erro genérico de coluna.
IF EXISTS(SELECT 1 FROM silver.pessoa_atributo_observacao WHERE atributo_codigo='EMAIL_CONTATO' AND ref.fn_email_canonico_v2(valor) IS NULL)
 THROW 51087,'Upgrade de EMAIL_CONTATO encontrou valor legado incompatível com EMAIL_CANONICO_V2; saneie o valor antes de prosseguir.',1;
UPDATE silver.pessoa_atributo_observacao
SET atributo_instancia_chave=CASE atributo_codigo
 WHEN 'TELEFONE_CONTATO' THEN CASE WHEN LEN(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(LTRIM(RTRIM(valor)),'+',''),' ',''),'-',''),'(',''),')',''),'.',''),'/',''),CHAR(9),''),CHAR(160),'')) IN(10,11) THEN '55'+REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(LTRIM(RTRIM(valor)),'+',''),' ',''),'-',''),'(',''),')',''),'.',''),'/',''),CHAR(9),''),CHAR(160),'') ELSE REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(LTRIM(RTRIM(valor)),'+',''),' ',''),'-',''),'(',''),')',''),'.',''),'/',''),CHAR(9),''),CHAR(160),'') END
 WHEN 'EMAIL_CONTATO' THEN ref.fn_email_canonico_v2(valor)
 ELSE '#' END
WHERE atributo_instancia_chave IS NULL;
ALTER TABLE silver.pessoa_atributo_observacao ALTER COLUMN atributo_instancia_chave NVARCHAR(512) NOT NULL;
IF NOT EXISTS(SELECT 1 FROM sys.default_constraints dc JOIN sys.columns c ON c.object_id=dc.parent_object_id AND c.column_id=dc.parent_column_id WHERE dc.parent_object_id=OBJECT_ID('silver.pessoa_atributo_observacao') AND c.name='atributo_instancia_chave') ALTER TABLE silver.pessoa_atributo_observacao ADD CONSTRAINT df_silver_pat_instancia_upgrade DEFAULT('#') FOR atributo_instancia_chave;
IF NOT EXISTS(SELECT 1 FROM sys.indexes WHERE object_id=OBJECT_ID('silver.pessoa_atributo_observacao') AND name='IX_silver_pat_instancia')
 CREATE INDEX IX_silver_pat_instancia ON silver.pessoa_atributo_observacao(pessoa_observacao_id,atributo_codigo,atributo_instancia_chave);
-- v3.60: toda chave legada de telefone é recomputada a partir do valor original pela mesma semântica V2.
-- Isso cobre inclusive prefixo internacional 00, perdido pela chave TELEFONE_DIGITOS_V1.
IF EXISTS(SELECT 1 FROM silver.pessoa_atributo_observacao WHERE atributo_codigo='TELEFONE_CONTATO' AND ref.fn_telefone_br_canonico_v2(valor) IS NULL)
 THROW 51086,'Upgrade de TELEFONE_CONTATO encontrou valor legado incompatível com TELEFONE_BR_CANONICO_V2; saneie o valor antes de prosseguir.',1;
UPDATE silver.pessoa_atributo_observacao
SET atributo_instancia_chave=ref.fn_telefone_br_canonico_v2(valor)
WHERE atributo_codigo='TELEFONE_CONTATO'
  AND atributo_instancia_chave<>ref.fn_telefone_br_canonico_v2(valor);

-- v3.68: migração de EMAIL_CASEFOLD_V1 para EMAIL_CANONICO_V2 é fail-closed e sempre parte do valor original.
IF EXISTS(SELECT 1 FROM silver.pessoa_atributo_observacao WHERE atributo_codigo='EMAIL_CONTATO' AND ref.fn_email_canonico_v2(valor) IS NULL)
 THROW 51087,'Upgrade de EMAIL_CONTATO encontrou valor legado incompatível com EMAIL_CANONICO_V2; saneie o valor antes de prosseguir.',1;
UPDATE silver.pessoa_atributo_observacao
SET atributo_instancia_chave=ref.fn_email_canonico_v2(valor)
WHERE atributo_codigo='EMAIL_CONTATO'
  AND atributo_instancia_chave<>ref.fn_email_canonico_v2(valor);
GO
...
IF OBJECT_ID('identidade.pessoa','U') IS NULL CREATE TABLE identidade.pessoa(
 pessoa_uuid UNIQUEIDENTIFIER PRIMARY KEY, status NVARCHAR(30) NOT NULL, pessoa_uuid_sucessor UNIQUEIDENTIFIER NULL, criado_em DATETIMEOFFSET(7) NOT NULL DEFAULT(SYSDATETIMEOFFSET()),
 CONSTRAINT fk_identidade_pessoa_sucessor FOREIGN KEY(pessoa_uuid_sucessor) REFERENCES identidade.pessoa(pessoa_uuid),
 CONSTRAINT ck_identidade_pessoa_sucessor CHECK(pessoa_uuid_sucessor IS NULL OR pessoa_uuid_sucessor<>pessoa_uuid),
 CONSTRAINT ck_identidade_pessoa_status CHECK(status IN('ATIVO','FUNDIDO','SEPARADO','INATIVO')));
GO
IF COL_LENGTH('identidade.pessoa','pessoa_uuid_sucessor') IS NULL ALTER TABLE identidade.pessoa ADD pessoa_uuid_sucessor UNIQUEIDENTIFIER NULL;
IF NOT EXISTS(SELECT 1 FROM sys.foreign_keys WHERE parent_object_id=OBJECT_ID('identidade.pessoa') AND name='fk_identidade_pessoa_sucessor') ALTER TABLE identidade.pessoa ADD CONSTRAINT fk_identidade_pessoa_sucessor FOREIGN KEY(pessoa_uuid_sucessor) REFERENCES identidade.pessoa(pessoa_uuid);
IF NOT EXISTS(SELECT 1 FROM sys.check_constraints WHERE parent_object_id=OBJECT_ID('identidade.pessoa') AND name='ck_identidade_pessoa_sucessor') ALTER TABLE identidade.pessoa WITH CHECK ADD CONSTRAINT ck_identidade_pessoa_sucessor CHECK(pessoa_uuid_sucessor IS NULL OR pessoa_uuid_sucessor<>pessoa_uuid);
IF NOT EXISTS(SELECT 1 FROM sys.indexes WHERE object_id=OBJECT_ID('identidade.pessoa') AND name='IX_identidade_pessoa_sucessor') CREATE INDEX IX_identidade_pessoa_sucessor ON identidade.pessoa(pessoa_uuid_sucessor) WHERE pessoa_uuid_sucessor IS NOT NULL;
GO
CREATE OR ALTER FUNCTION identidade.fn_pessoa_uuid_canonico(@pessoa_uuid UNIQUEIDENTIFIER)
RETURNS UNIQUEIDENTIFIER
AS
BEGIN
 DECLARE @atual UNIQUEIDENTIFIER=@pessoa_uuid,@proximo UNIQUEIDENTIFIER,@passos INT=0;
 IF NOT EXISTS(SELECT 1 FROM identidade.pessoa WHERE pessoa_uuid=@atual) RETURN NULL;
 WHILE @passos<32
 BEGIN
   SET @proximo=NULL;
   SELECT @proximo=pessoa_uuid_sucessor FROM identidade.pessoa WHERE pessoa_uuid=@atual;
   IF @proximo IS NULL RETURN @atual;
   SET @atual=@proximo; SET @passos+=1;
 END;
 RETURN NULL;
END;
GO
...
BEGIN
 ...
 ;WITH candidatos AS(
   SELECT vc.pessoa_uuid,pao.*,COALESCE(pao.referencia_evidencia,pao.verificado_em) precedencia,
          ROW_NUMBER() OVER(PARTITION BY vc.pessoa_uuid,pao.atributo_codigo,pao.atributo_instancia_chave ORDER BY COALESCE(pao.referencia_evidencia,pao.verificado_em) DESC,pao.verificado_em DESC,pao.pessoa_atributo_observacao_id DESC) rn
 ...
 INSERT gold.pessoa_atributo(pessoa_uuid,atributo_codigo,atributo_instancia_chave,valor,fonte_gestor_id,pessoa_atributo_observacao_id,source_record_id,evidencia_tipo,referencia_evidencia,verificado_em,precedencia_em,vigencia_inicio,vigencia_fim,atualizado_em)
 SELECT pessoa_uuid,atributo_codigo,atributo_instancia_chave,valor,fonte_gestor_id,pessoa_atributo_observacao_id,source_record_id,evidencia_tipo,referencia_evidencia,verificado_em,precedencia,@agora,NULL,@agora FROM candidatos WHERE rn=1;
...
-- Histórico SCD2 dos atributos transversais. Novo atributo SINGLE ou MULTI com chave já suportada = catálogo/config; nova regra de chave exige evolução explícita.
IF OBJECT_ID('gold.pessoa_atributo','U') IS NULL CREATE TABLE gold.pessoa_atributo(
 ...
 pessoa_uuid UNIQUEIDENTIFIER NOT NULL REFERENCES identidade.pessoa(pessoa_uuid),
 atributo_codigo NVARCHAR(80) NOT NULL REFERENCES ref.atributo_transversal(atributo_codigo),
 atributo_instancia_chave NVARCHAR(512) NOT NULL CONSTRAINT df_gold_pat_instancia DEFAULT('#'),
 valor NVARCHAR(2000) NOT NULL,
 fonte_gestor_id BIGINT NOT NULL REFERENCES ref.gestor(gestor_id),
...
GO
IF COL_LENGTH('gold.pessoa_atributo','atributo_instancia_chave') IS NULL
 ALTER TABLE gold.pessoa_atributo ADD atributo_instancia_chave NVARCHAR(512) NULL;
-- Fail-closed ANTES do backfill/ALTER NOT NULL, mantendo upgrade e replay sob a mesma regra corrente.
IF EXISTS(SELECT 1 FROM gold.pessoa_atributo WHERE atributo_codigo='EMAIL_CONTATO' AND ref.fn_email_canonico_v2(valor) IS NULL)
 THROW 51087,'Upgrade de EMAIL_CONTATO encontrou valor Gold legado incompatível com EMAIL_CANONICO_V2; saneie o valor antes de prosseguir.',1;
UPDATE ga SET atributo_instancia_chave=COALESCE(pao.atributo_instancia_chave,CASE ga.atributo_codigo WHEN 'TELEFONE_CONTATO' THEN CASE WHEN LEN(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(LTRIM(RTRIM(ga.valor)),'+',''),' ',''),'-',''),'(',''),')',''),'.',''),'/', ''),CHAR(9),''),CHAR(160),'')) IN(10,11) THEN '55'+REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(LTRIM(RTRIM(ga.valor)),'+',''),' ',''),'-',''),'(',''),')',''),'.',''),'/', ''),CHAR(9),''),CHAR(160),'') ELSE REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(LTRIM(RTRIM(ga.valor)),'+',''),' ',''),'-',''),'(',''),')',''),'.',''),'/', ''),CHAR(9),''),CHAR(160),'') END WHEN 'EMAIL_CONTATO' THEN ref.fn_email_canonico_v2(ga.valor) ELSE '#' END)
FROM gold.pessoa_atributo ga LEFT JOIN silver.pessoa_atributo_observacao pao ON pao.pessoa_atributo_observacao_id=ga.pessoa_atributo_observacao_id
WHERE ga.atributo_instancia_chave IS NULL;
ALTER TABLE gold.pessoa_atributo ALTER COLUMN atributo_instancia_chave NVARCHAR(512) NOT NULL;
IF NOT EXISTS(SELECT 1 FROM sys.default_constraints dc JOIN sys.columns c ON c.object_id=dc.parent_object_id AND c.column_id=dc.parent_column_id WHERE dc.parent_object_id=OBJECT_ID('gold.pessoa_atributo') AND c.name='atributo_instancia_chave') ALTER TABLE gold.pessoa_atributo ADD CONSTRAINT df_gold_pat_instancia_upgrade DEFAULT('#') FOR atributo_instancia_chave;
IF EXISTS (SELECT 1 FROM sys.indexes WHERE object_id=OBJECT_ID('gold.pessoa_atributo') AND name='UX_gold_pessoa_atributo_corrente') DROP INDEX UX_gold_pessoa_atributo_corrente ON gold.pessoa_atributo;
-- v3.60: Gold converge para a chave V2 calculada do valor original/observação, inclusive chaves legadas iniciadas por 00.
IF EXISTS(SELECT 1 FROM gold.pessoa_atributo WHERE atributo_codigo='TELEFONE_CONTATO' AND ref.fn_telefone_br_canonico_v2(valor) IS NULL)
 THROW 51086,'Upgrade de TELEFONE_CONTATO encontrou valor Gold legado incompatível com TELEFONE_BR_CANONICO_V2; saneie o valor antes de prosseguir.',1;
UPDATE ga SET atributo_instancia_chave=COALESCE(pao.atributo_instancia_chave,ref.fn_telefone_br_canonico_v2(ga.valor)),atualizado_em=SYSDATETIMEOFFSET()
FROM gold.pessoa_atributo ga LEFT JOIN silver.pessoa_atributo_observacao pao ON pao.pessoa_atributo_observacao_id=ga.pessoa_atributo_observacao_id
WHERE ga.atributo_codigo='TELEFONE_CONTATO'
  AND ga.atributo_instancia_chave<>COALESCE(pao.atributo_instancia_chave,ref.fn_telefone_br_canonico_v2(ga.valor));

IF EXISTS(SELECT 1 FROM gold.pessoa_atributo WHERE atributo_codigo='EMAIL_CONTATO' AND ref.fn_email_canonico_v2(valor) IS NULL)
 THROW 51087,'Upgrade de EMAIL_CONTATO encontrou valor Gold legado incompatível com EMAIL_CANONICO_V2; saneie o valor antes de prosseguir.',1;
UPDATE ga SET atributo_instancia_chave=ref.fn_email_canonico_v2(ga.valor),atualizado_em=SYSDATETIMEOFFSET()
FROM gold.pessoa_atributo ga
WHERE ga.atributo_codigo='EMAIL_CONTATO'
  AND ga.atributo_instancia_chave<>ref.fn_email_canonico_v2(ga.valor);

-- Se representações legadas distintas convergirem em V2, conserva a evidência de maior precedência
-- e encerra apenas duplicatas lógicas correntes de telefone/e-mail antes de recriar o índice único.
;WITH d AS(
 SELECT pessoa_atributo_id,ROW_NUMBER() OVER(PARTITION BY pessoa_uuid,atributo_codigo,atributo_instancia_chave ORDER BY precedencia_em DESC,pessoa_atributo_id DESC) rn
 FROM gold.pessoa_atributo WHERE vigencia_fim IS NULL AND atributo_codigo IN('TELEFONE_CONTATO','EMAIL_CONTATO'))
UPDATE ga SET vigencia_fim=CASE WHEN SYSDATETIMEOFFSET()<ga.vigencia_inicio THEN ga.vigencia_inicio ELSE SYSDATETIMEOFFSET() END,atualizado_em=SYSDATETIMEOFFSET()
FROM gold.pessoa_atributo ga JOIN d ON d.pessoa_atributo_id=ga.pessoa_atributo_id WHERE d.rn>1;
CREATE UNIQUE INDEX UX_gold_pessoa_atributo_corrente ON gold.pessoa_atributo(pessoa_uuid,atributo_codigo,atributo_instancia_chave) WHERE vigencia_fim IS NULL;
GO
CREATE OR ALTER VIEW gold.v_pessoa_atributo_corrente AS
SELECT pessoa_atributo_id,pessoa_uuid,atributo_codigo,atributo_instancia_chave,valor,fonte_gestor_id,pessoa_atributo_observacao_id,
...
BEGIN
 ...
 DECLARE @gestor BIGINT=(SELECT gestor_id FROM ref.gestor WHERE codigo=@gestor_codigo AND ativo=1),@motivo NVARCHAR(50);
 IF @gestor IS NULL THROW 51111,'Gestor responsável inexistente/inativo.',1;
 SELECT @motivo=motivo FROM identidade.caso_conflito_identidade WITH(UPDLOCK,HOLDLOCK) WHERE caso_id=@caso_id AND gestor_responsavel_id=@gestor AND status='ABERTO';
 IF @motivo IS NULL THROW 51112,'Caso inexistente, não pertence ao Gestor ou não está ABERTO.',1;
 ...
 -- Se o consumidor indicar um UUID que já foi fundido, a correção converge para o sucessor canônico.
 UPDATE g SET dest=c.canonico FROM @g g CROSS APPLY(SELECT identidade.fn_pessoa_uuid_canonico(g.dest) canonico)c WHERE c.canonico IS NOT NULL;
 -- v3.68: 51114 precisa ser avaliado ANTES do INSERT em @o, cuja PK(obs) mascararia o contrato com erro 2627/2601.
 ...
 -- FUSAO_HISTORICA é exaustiva por UUID absorvido e 2→1: todas as observações da origem convergem ao mesmo destino.
 IF @motivo='FUSAO_HISTORICA' AND (
   EXISTS(SELECT 1 FROM @o o WHERE o.uuid_ant IS NOT NULL GROUP BY o.uuid_ant HAVING COUNT(DISTINCT CONVERT(NVARCHAR(36),o.dest))>1)
   OR EXISTS(
     SELECT 1
     FROM (SELECT DISTINCT uuid_ant origem,dest FROM @o WHERE uuid_ant IS NOT NULL AND uuid_ant<>dest) c
     JOIN identidade.v_vinculo_corrente v ON v.pessoa_uuid=c.origem AND v.status='RESOLVIDO'
     WHERE NOT EXISTS(SELECT 1 FROM @o x WHERE x.uuid_ant=c.origem AND x.obs=v.pessoa_observacao_id AND x.dest=c.dest)))
   THROW 51119,'Fusão histórica incompleta: o UUID a absorver deve ter todas as observações incluídas integralmente no mesmo destino.',1;
 DECLARE @pre_fundidos TABLE(origem UNIQUEIDENTIFIER PRIMARY KEY,dest UNIQUEIDENTIFIER NOT NULL);
 IF @motivo='FUSAO_HISTORICA'
 BEGIN
   INSERT @pre_fundidos(origem,dest) SELECT DISTINCT uuid_ant,dest FROM @o WHERE uuid_ant IS NOT NULL AND uuid_ant<>dest;
   -- Preflight antes de qualquer mutação da aplicação. Destino novo é válido; cadeia existente inválida/cíclica não é.
   IF EXISTS(SELECT 1 FROM @pre_fundidos f JOIN identidade.pessoa p ON p.pessoa_uuid=f.dest WHERE identidade.fn_pessoa_uuid_canonico(f.dest) IS NULL OR identidade.fn_pessoa_uuid_canonico(f.dest)=f.origem)
     THROW 51117,'Fusão histórica produziria sucessão canônica inválida/cíclica.',1;
   IF EXISTS(
     SELECT 1 FROM identidade.identity_map src JOIN @pre_fundidos f ON f.origem=src.pessoa_uuid
     JOIN identidade.identity_map dst ON dst.pessoa_uuid=f.dest AND dst.tipo='CPF' AND dst.vigencia_fim IS NULL AND dst.estado IN('ATIVO','EM_CONFLITO')
     WHERE src.tipo='CPF' AND src.vigencia_fim IS NULL AND src.estado IN('ATIVO','EM_CONFLITO') AND src.identificador<>dst.identificador)
     THROW 51118,'Fusão histórica encontrou CPFs ativos divergentes entre UUID absorvido e canônico; resolva o conflito de CPF antes da fusão.',1;
 END;
 ...
 UPDATE p SET status='ATIVO',pessoa_uuid_sucessor=NULL FROM identidade.pessoa p JOIN @g g ON g.dest=p.pessoa_uuid;
 ...
 DECLARE @agora DATETIMEOFFSET(7)=SYSDATETIMEOFFSET();
 UPDATE b SET pessoa_uuid=o.dest,estado_atribuicao_identidade='ATRIBUIDA',atualizado_em=@agora
 ...
 UPDATE sv SET pessoa_uuid=o.dest,estado_atribuicao_identidade='ATRIBUIDA',atualizado_em=@agora
 FROM gold.servico_prestado sv JOIN silver.registro_observacao ro ON ro.registro_observacao_id=sv.registro_observacao_id JOIN @o o ON o.obs=ro.pessoa_observacao_id;
 UPDATE ri SET pessoa_uuid=o.dest,estado_atribuicao_identidade='ATRIBUIDA',atualizado_em=@agora
 ...
 DECLARE @afetados TABLE(uuid UNIQUEIDENTIFIER PRIMARY KEY);
 INSERT @afetados SELECT DISTINCT dest FROM @o UNION SELECT DISTINCT uuid_ant FROM @o WHERE uuid_ant IS NOT NULL;
 DECLARE @convergencia TABLE(origem UNIQUEIDENTIFIER PRIMARY KEY,dest UNIQUEIDENTIFIER NOT NULL);
 INSERT @convergencia(origem,dest)
 SELECT src.uuid_ant,d.dest
 FROM (SELECT DISTINCT uuid_ant FROM @o WHERE uuid_ant IS NOT NULL) src
 CROSS APPLY(SELECT TOP(1) dest FROM @o WHERE uuid_ant=src.uuid_ant ORDER BY CONVERT(varchar(36),dest)) d
 WHERE NOT EXISTS(SELECT 1 FROM @o x WHERE x.uuid_ant=src.uuid_ant AND x.dest<>d.dest);
 DECLARE @fundidos TABLE(origem UNIQUEIDENTIFIER PRIMARY KEY,dest UNIQUEIDENTIFIER NOT NULL);
 IF @motivo='FUSAO_HISTORICA'
   INSERT @fundidos(origem,dest) SELECT origem,dest FROM @pre_fundidos;

 -- Restabelece identificadores dos UUIDs que continuam vivos ou são destinos da correção, com evento auditável.
 DECLARE @mapas_reativados TABLE(identity_map_id BIGINT PRIMARY KEY);
 UPDATE im SET estado='ATIVO',estado_motivo='CASO_GOVERNADO_APLICADO',estado_em=@agora
 OUTPUT inserted.identity_map_id INTO @mapas_reativados(identity_map_id)
 FROM identidade.identity_map im
 WHERE im.vigencia_fim IS NULL AND im.estado='EM_CONFLITO'
   AND (EXISTS(SELECT 1 FROM @g g WHERE g.dest=im.pessoa_uuid)
        OR EXISTS(SELECT 1 FROM identidade.v_vinculo_corrente v WHERE v.pessoa_uuid=im.pessoa_uuid AND v.status='RESOLVIDO'));
 INSERT identidade.identity_map_estado_evento(identity_map_id,estado_anterior,estado_novo,motivo,referencia_operacao)
 SELECT identity_map_id,'EM_CONFLITO','ATIVO','CASO_GOVERNADO_APLICADO',@caso_id FROM @mapas_reativados;

 -- Em FUSAO_HISTORICA, identificadores ativos do UUID absorvido passam ao UUID canônico.
 -- CPF divergente já foi recusado no preflight, antes das mutações.
 DECLARE @mapas TABLE(identity_map_id BIGINT PRIMARY KEY,origem UNIQUEIDENTIFIER NOT NULL,dest UNIQUEIDENTIFIER NOT NULL,tipo NVARCHAR(30) NOT NULL,identificador NVARCHAR(255) NOT NULL,gestor_origem_id BIGINT NULL,source_record_id NVARCHAR(255) NULL);
 INSERT @mapas(identity_map_id,origem,dest,tipo,identificador,gestor_origem_id,source_record_id)
 SELECT im.identity_map_id,f.origem,f.dest,im.tipo,im.identificador,im.gestor_origem_id,im.source_record_id
 FROM identidade.identity_map im JOIN @fundidos f ON f.origem=im.pessoa_uuid
 WHERE im.vigencia_fim IS NULL;
 UPDATE im SET vigencia_fim=@agora,estado='ENCERRADO',estado_motivo='FUSAO_HISTORICA',estado_em=@agora
 FROM identidade.identity_map im JOIN @mapas m ON m.identity_map_id=im.identity_map_id;
 INSERT identidade.identity_map_estado_evento(identity_map_id,estado_anterior,estado_novo,motivo,referencia_operacao)
 SELECT m.identity_map_id,'EM_CONFLITO','ENCERRADO','FUSAO_HISTORICA',@caso_id FROM @mapas m;
 INSERT identidade.identity_map(pessoa_uuid,tipo,identificador,vigencia_inicio,gestor_origem_id,source_record_id,metodo_resolucao,estado,estado_motivo,estado_em)
 SELECT m.dest,m.tipo,m.identificador,@agora,m.gestor_origem_id,m.source_record_id,'CORRECAO_GOVERNADA','ATIVO','FUSAO_HISTORICA',@agora
 FROM @mapas m
 WHERE NOT EXISTS(SELECT 1 FROM identidade.identity_map x WHERE x.pessoa_uuid=m.dest AND x.tipo=m.tipo AND x.identificador=m.identificador AND x.vigencia_fim IS NULL AND x.estado='ATIVO');
 INSERT identidade.identity_map_estado_evento(identity_map_id,estado_anterior,estado_novo,motivo,referencia_operacao)
 SELECT im.identity_map_id,NULL,'ATIVO','FUSAO_HISTORICA',@caso_id
 FROM identidade.identity_map im JOIN @mapas m ON m.dest=im.pessoa_uuid AND m.tipo=im.tipo AND m.identificador=im.identificador
 WHERE im.vigencia_inicio=@agora AND im.estado='ATIVO' AND im.estado_motivo='FUSAO_HISTORICA';

 UPDATE p SET status='FUNDIDO',pessoa_uuid_sucessor=f.dest FROM identidade.pessoa p JOIN @fundidos f ON f.origem=p.pessoa_uuid;
 UPDATE p SET status='ATIVO',pessoa_uuid_sucessor=NULL FROM identidade.pessoa p JOIN @afetados a ON a.uuid=p.pessoa_uuid
 WHERE NOT EXISTS(SELECT 1 FROM @fundidos f WHERE f.origem=p.pessoa_uuid)
   AND EXISTS(SELECT 1 FROM identidade.v_vinculo_corrente v WHERE v.pessoa_uuid=p.pessoa_uuid AND v.status='RESOLVIDO');
 UPDATE p SET status='SEPARADO',pessoa_uuid_sucessor=NULL FROM identidade.pessoa p JOIN @afetados a ON a.uuid=p.pessoa_uuid
 WHERE NOT EXISTS(SELECT 1 FROM @fundidos f WHERE f.origem=p.pessoa_uuid)
   AND NOT EXISTS(SELECT 1 FROM identidade.v_vinculo_corrente v WHERE v.pessoa_uuid=p.pessoa_uuid AND v.status='RESOLVIDO')
   AND NOT EXISTS(SELECT 1 FROM identidade.identity_map m WHERE m.pessoa_uuid=p.pessoa_uuid AND m.vigencia_fim IS NULL);

 -- Atributos seguem a atribuição canônica. SINGLE concorre por código; MULTI concorre por código + chave de instância.
 UPDATE ga SET vigencia_fim=COALESCE(ga.vigencia_fim,@agora),atualizado_em=@agora FROM gold.pessoa_atributo ga JOIN @afetados a ON a.uuid=ga.pessoa_uuid WHERE ga.vigencia_fim IS NULL;
 UPDATE ga SET pessoa_uuid=o.dest,atualizado_em=@agora FROM gold.pessoa_atributo ga JOIN silver.pessoa_atributo_observacao pao ON pao.pessoa_atributo_observacao_id=ga.pessoa_atributo_observacao_id JOIN @o o ON o.obs=pao.pessoa_observacao_id;
 ;WITH candidatos AS(
   SELECT vc.pessoa_uuid,pao.*,COALESCE(pao.referencia_evidencia,pao.verificado_em) precedencia,
 ...
 UPDATE d SET status='RESOLVIDA',desfecho='CORRECAO_GOVERNADA',observacao_desfecho='Resolvida por caso governado de identidade.',encerrada_em=@agora,correlation_id=COALESCE(d.correlation_id,@correlation_id)
 ...
 UPDATE identidade.caso_conflito_identidade SET status='APLICADO',aplicado_em=@agora,correlation_id=COALESCE(correlation_id,@correlation_id) WHERE caso_id=@caso_id;
 IF @jornada_own_tran=1 COMMIT TRANSACTION;
 END TRY
 BEGIN CATCH
   IF @jornada_own_tran=1 AND XACT_STATE()<>0 ROLLBACK TRANSACTION;
