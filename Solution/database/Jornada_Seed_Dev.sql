...
SELECT g.gestor_id,4,'2026-09-13',CONCAT('config/contracts/gestores/',g.codigo,'/pessoa/v4/pessoa.schema.json'),
       CASE g.codigo WHEN 'SEHAB' THEN 0xbc62c7c3abb76a43d1a89b5b254c3179948d819de76fcf93c9c43960299467ba WHEN 'SMADS' THEN 0x4ba55cc1ab7f6edf22b38911a92f04ba5b88eb91ae1c1b9c08e6a0aeb0ffed0a WHEN 'SMDET' THEN 0xb7b6d5d05d0b68550b63d93bf7616556526ef059a72874c4c25da541e334679d WHEN 'SMS' THEN 0xcbf6a1751ef3666469a90c6fd906aa2e03f00cb7de9593c39446d7214938d60f END,'ATIVA','2026-09-13'
FROM ref.gestor g WHERE g.codigo IN('SMS','SEHAB','SMADS','SMDET')
...
UPDATE v SET pessoa_schema_ref=CONCAT('config/contracts/gestores/',g.codigo,'/pessoa/v4/pessoa.schema.json'),
             pessoa_schema_sha256=CASE g.codigo WHEN 'SEHAB' THEN 0xbc62c7c3abb76a43d1a89b5b254c3179948d819de76fcf93c9c43960299467ba WHEN 'SMADS' THEN 0x4ba55cc1ab7f6edf22b38911a92f04ba5b88eb91ae1c1b9c08e6a0aeb0ffed0a WHEN 'SMDET' THEN 0xb7b6d5d05d0b68550b63d93bf7616556526ef059a72874c4c25da541e334679d WHEN 'SMS' THEN 0xcbf6a1751ef3666469a90c6fd906aa2e03f00cb7de9593c39446d7214938d60f END,status='ATIVA',vigencia_inicio='2026-09-13',vigencia_fim=NULL,ativado_em=COALESCE(v.ativado_em,'2026-09-13')
FROM ref.gestor_pessoa_versao v JOIN ref.gestor g ON g.gestor_id=v.gestor_id
