-- Vocabulário progressivo V2. Aplicar após a persistência V1.
-- Migração transacional, reentrante e sem alteração de recibos históricos.
SET XACT_ABORT ON;
GO
...
-- A troca da guarda e a normalização são atômicas. Nenhum escritor observa a janela.
DROP TRIGGER identidade.tr_progressiva_origem_guard;
...
-- Histórico legado permanece íntegro. A nova guarda aceita sua equivalência sem reescrevê-lo.
...
EXEC(N'CREATE TRIGGER identidade.tr_progressiva_origem_guard ON identidade.pessoa_origem_progressiva AFTER UPDATE,DELETE AS
BEGIN
 SET NOCOUNT ON;
