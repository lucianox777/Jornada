...
## Replay dos mesmos pares IBGE — extensão do exportador existente (PR #507)

`CalibrationAuditExporter.ExportIbgeSyntheticReplayAsync` reutiliza a classe de exportação C# já integrada, mas **não** exporta modelo `ATIVO` nem observações SQL: seu SQL só lê `ref` pública `CENSO2022_NOMES_BRASIL_V1` em `DB_NAME()=JornadaSyntheticDev` com extended property `Jornada.EnvironmentProfile=Development`. O modo CLI `--export-splink-ibge-replay <arquivo> --seed <int> --pairs <1..100000> --first-name-sex <TODOS|FEMININO>` gera `JORNADA_SPLINK_IBGE_U_REPLAY_V1` e SHA-256 do conteúdo, com `LeftName`, `RightName`, `CSharpState` e proveniência da marginal pública. `ReplayPairs` usa os **mesmos sorteios** de `IbgeNominalUBootstrapEstimator.Estimate`; o exportador reconcilia as quatro contagens com `Estimate()` na seed e no número de pares escolhidos antes de emitir o arquivo. A opção `--check-splink-ibge-replay` reimporta esse documento, compara todos os estados externos e emite relatório **diagnóstico sem persistência de modelos**, com TVD e discordâncias. O contrato histórico de nove pessoas `JORNADA_SPLINK_EXCHANGE_V1` continua separado e somente smoke.

O teste da fonte é o código/hash da referência nominal **ATIVA já internalizada** pelo loader verificado; um hash em JSON isolado não equivale a assinatura de origem ou integridade externa. O runner Python Splink ainda deve ser executado **fora do repositório principal** e devolver os mesmos pares; a disponibilidade do código C# não prova equivalência empírica do Splink. Não transportar `--export-calibration` de um modelo real para o runner externo. [Runbook](Linkage_Splink_External_Runbook.md).
