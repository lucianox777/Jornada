...
`local-validate-release.ps1` chama `local-clean.ps1` **sempre no início**, portanto não é necessário executar a limpeza separadamente para a validação canônica. `local-clean.ps1` continua disponível com o mesmo nome para limpeza manual: remove recursos Docker locais da Jornada, o volume SQL, `.env`, `TestResults`, `bin/obj` e o Bronze local; em `.local`, remove os artefatos descartáveis, mas **preserva `.local/fabric-sql-compatibility/connection.txt` quando esse arquivo existir**, para que a conexão Fabric configurada pelo operador não seja perdida. A imagem Docker do SQL Server também é preservada. O cache `.vs` é tentado em best-effort: se Visual Studio/Copilot mantiver um arquivo aberto, o script avisa e continua porque esse cache não participa do restore/build/test.
