...
**Universo aberto, uso controlado:** a identidade progressiva pode receber atributos adicionais em novas entregas e versões de contrato; os três campos probabilísticos iniciais (nome civil, filiação materna e nascimento) **não** são o limite permanente da Pessoa. CPF válido, quando disponível, mantém sua rota determinística e as regras de conflito. Filiação paterna, naturalidade e identificadores secundários (como RG/CNH) são candidatos a novas evidências conforme proveniência, confiabilidade, cobertura, contrato e avaliação específica; a mera presença de um campo não autoriza pontuação nem fusão. `codigoPessoaOrigem` continua opcional; `initial_uuid` é âncora de linhagem, nunca feature estatística.

**Distinguir três funções:** (a) armazenar e compor a melhor representação Gold por atributo; (b) exibir informações adequadas ao atendimento; (c) utilizar sinais aprovados para blocking, scoring ou decisão determinística. **Nome social** integra o cadastro e tem precedência de tratamento/exibição quando existente, preservando o nome civil e sua proveniência; não se torna automaticamente uma segunda evidência independente do nome civil nem deve causar contagem dupla. **Referência territorial** tem natureza e estado próprios (domiciliar, acolhimento, serviço de referência, pernoite etc.) e pode mudar sem mudança de pessoa: no contrato Pessoa v5 vigente não é evidência positiva de linkage. Seu uso futuro como contexto auxiliar ou eventual sinal exige decisão explícita, avaliação de estabilidade temporal, correlação, vieses e riscos de compartilhamento de endereço; não converter coabitação em identidade. Benefícios e serviços não pontuam identidade por conveniência. Cada nova feature exige contrato/versionamento, testes adversariais, avaliação independente, autorização e reprocessamento das observações afetadas, sem reescrever o passado.
