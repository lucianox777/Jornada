...
| DT-09 | Imediata | Unificar parâmetros e asserts de `VALIDATE`/`ACTIVATE` | Fonte única dos parâmetros obrigatórios; os dois gates conferem a mesma versão de tolerância, fingerprint do snapshot, evidência mais recente e estado `CONFORME`; mutação posterior e evidência divergente bloqueiam promoção. | Wiring e helper compartilhado verificados no código; regressão contratual reforçada neste PR. A tolerância governada segue pendente (DT-01). |
