...
|---|---|---|---|---|
| DT-01 | Imediata | Congelar `config/linkage/implementation-conference-tolerance.json` | Tolerância numérica definida **antes** de conferir resultados governados, justificada por análise independente, versionada e testada em cenários conformes/divergentes; decisão final exige equivalência exata. Nunca derivar a tolerância do primeiro resultado nem relaxá-la para passar no gate. | Configuração técnica 0,01 / V1_2026-09-26 congelada ex ante; ampliar caracterização independente do erro numérico e exercitar conferência governada em DEV antes de declarar DT-01 encerrada. |
| DT-09 | Imediata | Unificar parâmetros e asserts de `VALIDATE`/`ACTIVATE` | Fonte única dos parâmetros obrigatórios; os dois gates conferem a mesma versão de tolerância, fingerprint do snapshot, evidência mais recente e estado `CONFORME`; mutação posterior e evidência divergente bloqueiam promoção. | Wiring e helper compartilhado verificados no código; regressão contratual reforçada neste PR. A tolerância técnica V1 está congelada; caracterização independente e execução DEV seguem no DT-01. |
