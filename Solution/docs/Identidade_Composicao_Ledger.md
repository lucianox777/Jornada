...
Instalações novas usam `database/Jornada_Fase1_v3.70.sql`, que inclui `database/migrations/20260908_Identidade_Composicao_Ledger.sql` após a persistência progressiva. Em upgrades controlados, a migração pode ser aplicada pelo manifesto versionado. As instalações são repetíveis e não executam backfill nem varredura da população. Nenhum serviço, endpoint ou job é registrado nesta fatia.
...
Os testes do planejador e a integração em SQL Server 2022 verificam concorrência de reserva e registro, replay idêntico, rejeição de conteúdo divergente, propriedade das reservas, rollback sem Pessoa órfã, imutabilidade e conservação de CPF, vínculos, Gold e referências progressivas. A reinstalação sobre registros povoados exige evidência de preservação. CI verde valida esta fatia de armazenamento, não autoriza fusões reais ou Linkage probabilístico.