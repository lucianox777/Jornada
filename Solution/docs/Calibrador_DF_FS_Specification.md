...
A composição entre as distribuições publicadas `NOME × SOBRENOME` continua, por compatibilidade, sob `INDEPENDENT_FIRST_NAME_SURNAME_MARGINALS_V1`. O nome histórico da versão não autoriza decompor valores publicados em tokens: um valor `NOME` ou `SOBRENOME` com múltiplas palavras permanece uma unidade estatística. A independência assumida é somente entre as duas marginais publicadas. Eventual dependência entre elas deve ser estudada por sensibilidade/validação independente; não se introduz fator de correlação arbitrário no modelo canônico. A validação pré-HML deve ainda comparar artefatos congelados por `modelo_id`/fingerprints e repetir seeds para separar variabilidade Monte Carlo de efeito de mudança de modelo. Corpus sintético e adversarial servem para engenharia e falsificação; não constituem homologação populacional.
...
O recorte inicial usa somente as marginais nacionais publicadas `BRASIL/TODOS/TODOS` da referência versionada. Para tornar os estados `EXACT/HIGH/MEDIUM/LOW` comparáveis ao comparador nominal da Jornada, o bootstrap compõe sinteticamente um valor publicado de `NOME` com um valor publicado de `SOBRENOME`, por sorteios independentes ponderados pelas frequências da referência. Valores compostos já presentes em qualquer marginal são preservados integralmente. Essa composição é marcada por `INDEPENDENT_FIRST_NAME_SURNAME_MARGINALS_V1`: a hipótese metodológica é a independência entre as marginais `NOME` e `SOBRENOME`, não independência entre palavras dentro de um nome ou sobrenome composto. A auditoria `local-linkage-triplet-collision-audit.ps1` registra quantos valores multi-token estão efetivamente presentes no snapshot operacional, para que essa propriedade seja comprovada pela referência carregada e não assumida pela documentação.
...
A probabilidade de coincidência exata também é calculada analiticamente a partir das marginais publicadas (`sum(p_i^2)` para `NOME` e `SOBRENOME`; produto sob a hipótese sintética de independência entre as duas marginais) e funciona como controle do Monte Carlo. Os níveis fuzzy são estimados por amostragem determinística com seed explícita e comparador `IdentityComparison.CompareName` vigente.
...
A instrumentação persiste `DIAG_CANDIDATE_PRIOR_*`, inclusive tamanho da amostra, observações com candidatos, pares verdadeiros/falsos, prior empírico e delta de log-odds contra o prior ativo. `DIAG_CANDIDATE_PRIOR_ACTIVE_SCORE_CHANGED=0` é uma invariável desta versão: o diagnóstico não altera score, threshold, margem ou decisão.
