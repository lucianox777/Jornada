...
Fila institucional de divergências **abertas** do Gestor, limitada a 1..500 itens (`limit` padrão 100). Na Fase 1, a tipologia produzida automaticamente pela Solution é `DIVERGENCIA_IDENTIDADE`. Conflitos de Linkage entram nessa fila somente depois de um `linkage_run` ser efetivamente `PUBLICADO`; a divergência guarda `linkage_run_id` próprio e preserva em `correlation_id` a correlação original do run, permitindo auditoria interna do modelo, scores, margem e candidatos em `qualidade.v_divergencia_linkage_contexto`. A API não devolve CPF, UUID, scores nem conteúdo factual; entrega somente identificadores de observação/origem, tipologia, motivo, correlação e instante de abertura.
