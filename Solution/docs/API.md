...
Quando um ato constitui confirmação humana, `evidencia.tipo` aceita somente `DOCUMENTO_VERIFICADO` ou `CONFIRMACAO_INSTITUCIONAL_SEM_DOCUMENTO`. `DOCUMENTO_VERIFICADO` exige `evidencia.documentoTipoCodigo`. Abertura de caso não é confirmação; a evidência é informada no ato decisório/aplicação.
