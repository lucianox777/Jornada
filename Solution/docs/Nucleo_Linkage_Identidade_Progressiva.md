...
## Premissa de produto: melhor representação, não certeza

A Jornada é **sistema de apoio à decisão**: constrói a melhor representação estatística e incremental da identidade com as evidências disponíveis, explicitando incerteza, alternativas, conflitos, origem e versão. **Não concede, nega nem condiciona automaticamente benefícios ou serviços**, nem certifica a identidade civil. A responsabilidade finalística e a avaliação de documentos apresentados permanecem com a Secretaria competente, segundo suas normas. Estados operacionais e limiares controlam a publicação de hipóteses de vínculo, não atestam verdade absoluta. `REFERENCIA` designa uma representação canônica operacional, **não identidade comprovada ou definitiva**.

**Hierarquia de evidências por atributo para a Gold:** documentação apresentada mais recentemente > documentação apresentada anteriormente > autodeclaração mais recente > autodeclaração mais antiga. A recência é a da apresentação/registro da evidência **dentro da classe**, não a simples data de emissão do documento. Guardar apresentação, emissão, data do fato e validade separadamente quando disponíveis. Documento ainda válido não garante que cada atributo seja o mais atual ou esteja correto. Preservar divergências e histórico; a hierarquia orienta a representação corrente, não transforma documento em verdade absoluta. Não aplicar regra universal de validade de dez anos a todas as espécies documentais: verificar normas específicas quando necessário. Preservar versões e divergências sem substituir automaticamente a informação mais recente pela documental, nem o contrário. A Gold é a melhor representação **disponível**, revisável à luz de novas evidências; não é cadastro perfeito. A ausência de atributos não implica identidade inválida.
...
5. Comparar o resultado com a **representação/decisão semântica corrente**: status, UUID canônico, vínculo, motivo governado e alterações relevantes de confiança, alternativas ou conflitos conforme contrato versionado. Se semanticamente idêntico, não criar nova versão operacional da decisão; registrar a execução e evidência necessária em trilha separada. Se diferente, persistir a nova melhor representação e recompor somente as pessoas Gold afetadas, com atomicidade entre decisão, publicação e ledger quando exigida.
...
Preservar a confirmação governada atual até existir o caminho síncrono de busca sob demanda. O motor C# compartilhado selecionará **até cinco candidatos mais prováveis** usando somente sinais de identidade; a tela fará **conferência semicega**, com ordem independente do ranking, sem scores ou destaque, e com **“Nenhum destes”** sempre disponível. Não inventar candidatos para preencher cinco posições. O limite é **decisão de ergonomia**, plausível diante da literatura sobre capacidade central de aproximadamente quatro unidades de memória (Cowan, 2001), mas não um limite experimental demonstrado para a conferência de pessoas. Estudos sobre sobrecarga de escolha não estabelecem um número universal (Scheibehenne et al., 2010). Validar tempo, erros, omissões e recall@5 no Ensaio, sem exigir que o atendente memorize todas as fichas. Ver justificativa, referências e critérios em `Diretrizes_Identidade_Progressiva_Apoio_Decisao.md`.

Usar `FHIR Patient/$match` como referência de contrato de busca e retorno, com adaptador/perfil quando útil; não impor semântica clínica a todas as Secretarias. O contrato técnico pode conter score/classificação para sistemas autorizados, **não** para a apresentação semicega. A busca síncrona reutiliza scorer/blocking do Runner, mas não cria `linkage_run`, `identity_map` nem vínculo; precisa de autenticação, autorização, auditoria e limites. A seleção pelo atendente continua ato governado com autoria individual quando a identidade corporativa estiver integrada. Não usar confirmações de casos difíceis como amostra representativa para estimar `m` sem corrigir o desenho amostral.
