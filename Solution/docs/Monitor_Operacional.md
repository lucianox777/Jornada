...
### Conferência externa Splink × mesmo bootstrap IBGE — diagnóstico DEV

A [decisão consolidada §2.1](Decisoes_Linkage_Calibracao_IBGE_20260926.md#21-conferência-externa-jornada--splink--decisão-consolidada-de-26092026) substitui a ADR-007 como norma. O estado `linkageConferenceGovernance` acrescenta **`splinkMethod`**, **`splinkStatus`**, `splinkResultStatus`, `splinkReferenceSha256`, `splinkEvidenceSha256`, `splinkPairsEvaluated`, `splinkPairwiseDisagreements` e timestamp de atualização do arquivo externo, sem alterar os campos `status`/`evidenceId` da conferência governada nem `roundTripStatus` do exportador de modelos.

Na ausência de arquivos autorizados, a API retorna `SEM_EVIDENCIA_EXTERNA`. Somente em `ASPNETCORE_ENVIRONMENT=Development`, com `JORNADA_SPLINK_IBGE_INPUT_PATH` e `JORNADA_SPLINK_IBGE_RESULT_PATH` apontando para arquivos locais brutos de replay público/sintético, a API verifica ambos novamente, exige que o hash da referência bata com a referência IBGE **ATIVA** e calcula comparação de cada par. O estado exibido `DIAGNOSTICO_EXTERNO_NAO_GOVERNADO` **não** significa aprovação: `splinkResultStatus` distingue estados idênticos e divergentes. Entrada maior que 16 MB, saída maior que 10 MB, fonte diferente, arquivo ausente ou contrato inválido produzem estado explícito, **não** sucesso silencioso. O monitor não transporta nomes nem os parâmetros numéricos, não invoca Python, não recalibra e não publica evidência de aprovação para HML/Produção. Consulte o [runbook](Linkage_Splink_External_Runbook.md). A versão instalada declarada pelo runner no arquivo é informação técnica, **não prova automática de que ele foi executado independentemente**; hashes e log da execução externa continuam obrigatórios.
