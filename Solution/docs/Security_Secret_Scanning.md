...
## Transporte seguro de senha no orquestrador e diagnóstico de linkage (#405)

`local-linkage-validation-flow.ps1`/`.sh` e `linkage-evaluation-smoke.sh` passaram a encaminhar a senha via ambiente filho `SQLCMDPASSWORD`, passando somente `-e SQLCMDPASSWORD` ao Docker. A função `Get-SqlScalar` PowerShell restaura o ambiente original em `finally`, inclusive em exceções e na ausência inicial da variável. Os testes `local-test-linkage-flow-secret-transport.ps1` e `test-linkage-flow-evaluation-secret-transport.sh` extraem somente as funções reais, simulam Docker e verificam não exposição da senha em argumentos/log, propagação de falhas e restauração ambiental, sem executar a orquestração, ingestão, SQL ou limpezas reais. A integridade estatística do linkage e a autorização HML continuam gates independentes.
...
## Segredos temporários no probe da imagem SQL de release (#405)

`local-validate-release.ps1` gera uma senha exclusivamente para o contêiner efêmero de verificação da imagem SQL Server. O `docker run` passa apenas `-e MSSQL_SA_PASSWORD` e os dois `docker exec` passam apenas `-e SQLCMDPASSWORD`, com ambos os valores no ambiente temporário do processo chamador. A implementação restaura os estados originais de ambas as variáveis em `finally`, inclusive quando o Docker lança exceção ou quando as variáveis não existiam anteriormente; o contêiner de teste continua sendo removido ao final. `local-test-release-sql-probe-secret-transport.ps1` extrai as funções reais via AST, simula Docker em Windows PowerShell 5.1 e pwsh e verifica sucesso, imagem não inicializada, falha de criação, erro de consulta de versão e falha de limpeza sem executar Docker ou SQL de verdade. Essas simulações não substituem o probe real da imagem fixada por digest durante a validação de release. A issue #405 permanece aberta para rotinas remanescentes e revisão/rotação de credenciais.

## SQLCMDPASSWORD nos jobs SQL do CI (#405)

Os jobs `integration-sql` e `harness-smoke` mantêm senhas **efêmeras por `github.run_id`** somente para seus próprios serviços SQL Server. Os 19 comandos Docker/sqlcmd deixaram de transmitir `-P` e passam a usar atribuição temporária de `SQLCMDPASSWORD` no processo filho com `docker exec -e SQLCMDPASSWORD`. Os dois healthchecks SQL passam a ler `MSSQL_SA_PASSWORD` já existente **dentro dos contêineres**, sem expor o valor na linha de comando. O gate `test-ci-sqlcmd-secret-transport.py` inspeciona os 19 comandos e dois healthchecks e testa três regressões simuladas; a suíte CI continua exercitando SQL Server real e schema 3.70 em contêineres isolados. Senhas RC restantes são tratadas separadamente; esta alteração não transfere credenciais DEV para HML/PRD nem substitui a gestão corporativa (#378).

## RC: passagem de senha SQL por ambiente (#405)

O job `rc-evidence` mantém senha efêmera e o gate de provenance somente em tag/`workflow_dispatch` autorizado. As quatro chamadas SQL CLI, incluindo readiness do serviço, criação/carga do schema e fingerprint estrutural, agora recebem `SQLCMDPASSWORD` no ambiente temporário de cada processo, sem `-P` no argv nem alteração dos critérios RC. O teste `test-rc-sqlcmd-secret-transport.py` garante as quatro chamadas, o fingerprint e três mutações negativas, mas não substitui executar o gate real de RC em um preflight controlado. Sem reescrita de histórico, sem atualização de schema e sem alteração HML/PRD (#378).
