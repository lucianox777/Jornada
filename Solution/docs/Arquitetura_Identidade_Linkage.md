...
Um conflito probabilístico só ingressa na fila institucional depois de um `linkage_run` efetivamente `PUBLICADO`. A fila reutiliza `qualidade.divergencia_gestor`; um `linkage_run_id` explícito liga o caso ao resultado, modelo, scores, margem e candidatos persistidos, enquanto `correlation_id` mantém a correlação original do run, sem transformar a indicação em correção automática. Reexecução da sincronização do mesmo run é idempotente e uma divergência aberta preexistente para a observação não é duplicada.
