...
`POSITIONAL_TOKEN_MIN_JARO_WINKLER_V2` existe para impedir que uma substituição forte em um token seja diluída por um nome completo longo quase todo igual. Ele não atribui semântica IBGE aos tokens, não remove partículas e, quando a quantidade de tokens difere, mantém deliberadamente o comportamento V1 até existir evidência para uma política versionada de inserção/remoção/reordenação. `IdentityComparison.CompareName` permanece alias explícito de V1 para preservar replay; V2 só pode ser usada por chamada/algoritmo que a selecione explicitamente.
