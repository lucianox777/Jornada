...
internal static class LinkageModelPolicy
{
    ...
    internal static bool SupportsBirthComponentScoring(LinkageModel model)
    {
        ...
        var enabled = model.Parameters.TryGetValue(LinkageParameterCatalog.BirthComponentScoring, out var current)
            ? current
            : model.Parameters.TryGetValue(LinkageParameterCatalog.LegacyBirthComponentScoring, out var legacy) ? legacy : 0m;
        if (enabled < 1m)
            return false;
...
{
    internal static ProbabilisticLinkageDecision Resolve(LinkageModel model, IdentityObservation observation,
