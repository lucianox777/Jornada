...
/// <summary>Estima m/u com suavização. No V6, ausência de nome da mãe é estado explícito MISSING.</summary>
public static class LinkageParameterEstimator
{
    ...
    public static IReadOnlyDictionary<string, decimal> Estimate(IReadOnlyList<IdentityTrainingPair> matchedPairs, IReadOnlyList<IdentityTrainingPair> unmatchedPairs, long populationSize, long distinctBirthDates, decimal smoothingAlpha, decimal threshold, decimal conflictMargin, BirthScoringContract birthScoringContract = BirthScoringContract.SemanticEvidenceV5)
    {
        ...
        var scoringFlag = birthScoringContract switch { BirthScoringContract.SemanticEvidenceV5 => LinkageParameterCatalog.BirthSemanticEvidenceScoring, BirthScoringContract.JointEvidenceV4 => LinkageParameterCatalog.BirthJointEvidenceScoring, BirthScoringContract.SingleEvidenceV3 => LinkageParameterCatalog.BirthSingleEvidenceScoring, _ => throw new ArgumentOutOfRangeException(nameof(birthScoringContract)) };
        ...
        var result = new Dictionary<string, decimal>(StringComparer.Ordinal)
        {
            ...
            [LinkageParameterCatalog.LogOddsConflictMargin] = conflictMargin,
        ...
        AddMotherDistribution(result, "M_NOME_MAE", matchedPairs, smoothingAlpha);
        AddMotherDistribution(result, "U_NOME_MAE", unmatchedPairs, smoothingAlpha);
    ...
    private static void AddMotherDistribution(IDictionary<string, decimal> target, string prefix, IEnumerable<IdentityTrainingPair> pairs, decimal alpha)
    {
        var counts = LinkageParameterCatalog.MotherNameStates.ToDictionary(s => s, _ => 0L, StringComparer.Ordinal); long total = 0;
        foreach (var p in pairs)
        {
            var state = IdentityComparison.NormalizeText(p.LeftMotherName) is null || IdentityComparison.NormalizeText(p.RightMotherName) is null ? "MISSING" : IdentityComparison.CompareName(p.LeftMotherName, p.RightMotherName).ToString();
            counts[state]++; total++;
        }
        var denominator = total + alpha * counts.Count;
        foreach (var state in LinkageParameterCatalog.MotherNameStates) target[$"{prefix}_{state}"] = (counts[state] + alpha) / denominator;
    }
    ...
    {
        var counts = States.ToDictionary(s => s, _ => 0L); long total = 0;
        foreach (var state in values) { counts[state]++; total++; }
        var denominator = total + alpha * States.Length;
        foreach (var state in States) target[$"{prefix}_{state}"] = (counts[state] + alpha) / denominator;
    }
    ...
    {
        var counts = BirthDateSemanticEvidence.States.ToDictionary(s => s, _ => 0L, StringComparer.Ordinal); long total = 0;
        foreach (var state in values) { if (!counts.TryGetValue(state, out var count)) throw new InvalidOperationException($"Estado semântico de nascimento inválido: {state}."); counts[state] = count + 1; total++; }
        var denominator = total + alpha * counts.Count;
        foreach (var state in BirthDateSemanticEvidence.States) target[$"{prefix}_{state}"] = (counts[state] + alpha) / denominator;
    }
    ...
    private static void AddSemanticBirthSupport(IDictionary<string, decimal> target, string prefix, IEnumerable<string> values)
    {
        ...
        foreach (var state in values) { if (!counts.TryGetValue(state, out var count)) throw new InvalidOperationException($"Estado semântico de nascimento inválido: {state}."); counts[state] = count + 1; }
        foreach (var state in BirthDateSemanticEvidence.States) target[$"{prefix}_{state}"] = counts[state];
    }
    ...
    {
        var counts = new long[LinkageParameterCatalog.BirthJointStates.Count]; long total = 0;
        foreach (var value in values) { if (value >= counts.Length) throw new InvalidOperationException("Estado conjunto de nascimento inválido."); counts[value]++; total++; }
        var denominator = total + alpha * counts.Length;
        for (var i = 0; i < counts.Length; i++) target[$"{prefix}_{LinkageParameterCatalog.BirthJointStates[i]}"] = (counts[i] + alpha) / denominator;
    }
    ...
    private static void AddJointBirthSupport(IDictionary<string, decimal> target, string prefix, IEnumerable<byte> values)
    {
        ...
        foreach (var value in values) { if (value >= counts.Length) throw new InvalidOperationException("Estado conjunto de nascimento inválido."); counts[value]++; }
        for (var i = 0; i < counts.Length; i++) target[$"{prefix}_{LinkageParameterCatalog.BirthJointStates[i]}"] = counts[i];
    }

    private static byte BirthAgreementMask(IdentityTrainingPair p) => (byte)((p.LeftBirthDate.Day == p.RightBirthDate.Day ? 1 : 0) | (p.LeftBirthDate.Month == p.RightBirthDate.Month ? 2 : 0) | (p.LeftBirthDate.Year == p.RightBirthDate.Year ? 4 : 0));
    ...
    {
        long exact = 0, total = 0; foreach (var value in values) { if (value) exact++; total++; }
        var probability = (exact + alpha) / (total + 2m * alpha); target[$"{prefix}_EXACT"] = probability; target[$"{prefix}_DIFF"] = 1m - probability;
    }
