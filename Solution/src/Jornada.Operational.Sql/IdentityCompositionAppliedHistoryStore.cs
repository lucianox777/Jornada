...
public sealed class IdentityCompositionAppliedHistoryStore
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        foreach (var reference in ids)
        {
            ...
            while (await reader.ReadAsync(cancellationToken))
                decisions.Add(reader.GetGuid(0));
        }

        var result = new List<IdentityCompositionHistory>();
        foreach (var decisionId in decisions)
        {
            // Nunca aceitar uma linha isolada sem validar a decisão inteira e seu recibo.
    ...
    public async Task<ImmutableArray<IdentityCompositionHistory>> LoadDecisionAsync(
        DbConnection connection, DbTransaction transaction, Guid decisionId,
        CancellationToken cancellationToken = default)
    {
        ValidateTransaction(connection, transaction);
        if (decisionId == Guid.Empty)
            throw new ArgumentException("Decisão inválida.", nameof(decisionId));
        ...
        {
            throw new InvalidOperationException("Payload preparado inválido.", ex);
        }
        ...
        await using (var command = connection.CreateCommand())
        {
            ...
            while (await reader.ReadAsync(cancellationToken))
                rows.Add(new IdentityCompositionAppliedHistoryRow(reader.GetGuid(0), reader.GetGuid(1),
                    reader.GetString(2), reader.GetString(3), ReadUtcTimestamp(reader, 4), true));
        }
