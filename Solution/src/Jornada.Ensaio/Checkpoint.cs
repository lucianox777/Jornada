...
public sealed class CheckpointCollector(Func<DbConnection> abrirConexao)
{
    ...
    [
        ...
        ("modelos","Modelos de linkage","SELECT TOP 10 modelo_id,versao,status,CASE WHEN base_referencia='CI_LINKAGE_SYNTHETIC' THEN 1 ELSE 0 END sintetico,base_referencia,gerado_em FROM identidade.modelo_linkage ORDER BY gerado_em DESC"),
        ("ruleset_modelo","Proveniência modelo → ruleset","SELECT TOP 10 m.modelo_id,m.versao modelo_versao,m.status modelo_status,r.ruleset_id,r.ruleset_versao,r.algoritmo_versao,r.fingerprint_sha256,COUNT(DISTINCT p.passe_ordem) passes,COUNT(c.campo_ordem) campos FROM identidade.modelo_linkage m LEFT JOIN identidade.linkage_ruleset r ON r.modelo_id=m.modelo_id LEFT JOIN identidade.linkage_ruleset_passe p ON p.ruleset_id=r.ruleset_id LEFT JOIN identidade.linkage_ruleset_passe_campo c ON c.ruleset_id=p.ruleset_id AND c.passe_ordem=p.passe_ordem GROUP BY m.modelo_id,m.versao,m.status,m.gerado_em,r.ruleset_id,r.ruleset_versao,r.algoritmo_versao,r.fingerprint_sha256 ORDER BY m.gerado_em DESC"),
        ...
        ("linkage_runs","Execuções de linkage","SELECT TOP 10 linkage_run_id,modelo_id,modelo_versao,tipo_run,status,avaliados,iniciado_em,publicado_em FROM identidade.linkage_run ORDER BY iniciado_em DESC"),
