    ...
    ICollection<string> log)
{
    ...
    {
        var packages = Directory.GetFiles(options.PackagesDirectory, $"{stage.PrefixoArquivo}*.zip")
            .OrderBy(path => path, StringComparer.Ordinal)
            .ToArray();

        if (packages.Length == 0)
            throw new FileNotFoundException($"Nenhum pacote {stage.PrefixoArquivo}*.zip encontrado em {options.PackagesDirectory}.");
        ...
        foreach (var path in packages)
        {
            ...
            using var request = new HttpRequestMessage(HttpMethod.Post, options.IngestionEndpoint) { Content = content };
            ...
            else log.Add($"{stage.Codigo}: {name} -> {(int)response.StatusCode} {body}");
        }
