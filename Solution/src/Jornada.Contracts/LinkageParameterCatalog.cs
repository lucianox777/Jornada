...
public static class LinkageParameterCatalog
{
    ...
    public static readonly IReadOnlyList<string> NameStates =
    ...
    public static readonly IReadOnlyList<string> CalibrationValidationRequired =
        [
            .. CoreScoringRequired,
            MatchedSampleSize, UnmatchedSampleSize, PopulationSize, DistinctBirthDate
        ];

    public static string Name(string prefix, string state) => $"{prefix}_{state}";

    private static string[] Distribution(string prefix) =>
        NameStates.Select(state => Name(prefix, state)).ToArray();

    private static string[] BinaryDistribution(string prefix) =>
        [Name(prefix, "EXACT"), Name(prefix, "DIFF")];
}
