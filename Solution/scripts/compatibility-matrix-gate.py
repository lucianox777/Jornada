...
def main():
    ...
    if d.get('baseNormativa')!='3.62' or d.get('solutionSchema')!='3.69': fail('schema esperado divergente')
    cases={x['id']:x for x in d.get('cases',[])}
    need={'API_CURRENT_SCHEMA_CURRENT','API_CURRENT_SCHEMA_V368','UPGRADE_V368_TO_CURRENT','UPGRADE_V365_TO_CURRENT','DDL_REAPPLY_CURRENT'}
    if set(cases)!=need: fail('casos obrigatórios divergentes')
    h=HEALTH.read_text(encoding='utf-8')
    for token in ('N\'Jornada.BaseNormativa\'','N\'Jornada.SolutionSchema\'','N\'3.62\'','N\'3.69\'','SQL_SCHEMA_INCOMPATIVEL'):
    ...
    if cases['API_CURRENT_SCHEMA_V368']['expected']!='DENY_SQL_SCHEMA_INCOMPATIVEL': fail('schema 3.68 deve ser deny até upgrade')
    print('COMPATIBILITY MATRIX GATE: OK (current=3.69; v3.68/v3.65=upgrade-required; reapply=idempotent)')
    return 0
