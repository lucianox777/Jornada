...
def main():
    ...
    if d.get('baseNormativa')!='3.62' or d.get('solutionSchema')!='3.70': fail('schema esperado divergente')
    cases={x['id']:x for x in d.get('cases',[])}
    need={'API_CURRENT_SCHEMA_CURRENT','API_CURRENT_SCHEMA_V369','UPGRADE_V369_TO_CURRENT','UPGRADE_V368_TO_CURRENT','DDL_REAPPLY_CURRENT'}
    ...
    print('COMPATIBILITY MATRIX GATE: OK (current=3.70; v3.69/v3.68=upgrade-required; reapply=idempotent)')
    return 0
