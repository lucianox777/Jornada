...
function Wait-ForNextStep { param([Parameter(Mandatory=$true)][string]$CompletedStep); Write-Host ''; Write-Host "OK: $CompletedStep" -ForegroundColor Green; [void](Read-Host 'Pressione ENTER para executar o próximo comando'); Write-Host '' }
function Invoke-Step {
    ...
    if(-not $?){throw "Falha em: $Title"}; if($LASTEXITCODE-ne 0){throw "Falha em: $Title (exit code $LASTEXITCODE)"}; Wait-ForNextStep $Title
}

Push-Location $Root
try {
    Write-Host 'RODADA LOCAL DE TESTES - EXECUÇÃO PASSO A PASSO' -ForegroundColor Yellow
    Write-Host "Solution: $Root"; Write-Host 'O script imprime cada comando, para na primeira falha e pausa entre todos os comandos.'; Write-Host ''
