...
def main() -> None:
    ...
    metodo_final = "CHECK(metodo_resolucao IN('CPF_DETERMINISTICO','NIS_DETERMINISTICO','UUID_JORNADA_RETROALIMENTACAO','PENDENTE_PROBABILISTICO','LINKAGE_PROBABILISTICO','CORRECAO_GOVERNADA','CONFLITO_GOVERNADO'))"
    modelo_conflict = "(metodo_resolucao='CONFLITO_GOVERNADO' AND score IS NULL AND modelo_id IS NULL AND pessoa_uuid IS NULL)"
    modelo_nis = "(metodo_resolucao='NIS_DETERMINISTICO' AND score IS NULL AND modelo_id IS NULL)"
    ...
    if sql.count(metodo_final) < 2:
        fail("ck_vinculo_metodo: blocos de reentrada devem aceitar NIS_DETERMINISTICO, UUID_JORNADA_RETROALIMENTACAO e CONFLITO_GOVERNADO")
    if sql.count(modelo_conflict) < 2:
        fail("ck_vinculo_modelo: blocos de reentrada devem aceitar CONFLITO_GOVERNADO")
    if sql.count(modelo_nis) < 2:
        fail("ck_vinculo_modelo: blocos de reentrada devem aceitar NIS_DETERMINISTICO")
