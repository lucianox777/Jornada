...
function Scalar([string]$Query){ Push-Location $Root; try { $o = (& docker compose --env-file .env exec -T -e "SQLCMDPASSWORD=$pwd" sqlserver /opt/mssql-tools18/bin/sqlcmd -S localhost -U sa -C -b -d $db -h -1 -W -Q "SET NOCOUNT ON; $Query"); if($LASTEXITCODE -ne 0){throw 'sqlcmd falhou.'}; return ($o | ? { $_.Trim() } | Select-Object -Last 1).Trim() } finally { Pop-Location } }
...
[ordered]@{profile=$Profile;seed=$seed;collisionModulo=$collisionModulo;birthShiftModulo=$birthShiftModulo;goldPeople=$people;pairedPeople=$paired;pendingWithoutCpf=$pending;trainingSampleSize=$sample;trainingPoolSize=$pool;modelVersion=$model;parametersGenerateMilliseconds=$paramMs;runnerMilliseconds=$runnerMs;runner=[ordered]@{status=$row[0];eligible=[int64]$row[1];evaluated=[int64]$row[2];resolved=[int64]$row[3];unresolved=[int64]$row[4];conflicts=[int64]$row[5];noCandidateInBirthDateBlock=[int64]$row[6]};correlationId="$corr";generatedAtUtc=(Get-Date).ToUniversalTime().ToString('o')} | ConvertTo-Json -Depth 4 | Set-Content -Encoding utf8 $out
