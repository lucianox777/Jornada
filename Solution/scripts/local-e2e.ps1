...
$db = if ($usesTemporaryDatabase) {
    'JornadaE2E_' + [Guid]::NewGuid().ToString('N').Substring(0,12)
}
else {
    $sharedDb
}
...
if ($usesTemporaryDatabase) {
    Write-Host "E2E usando banco temporario isolado: $db"
...
try {
    ...
    if ($usesTemporaryDatabase) {
        Write-Host "Removendo banco E2E temporario: $db"
        Push-Location $Root
        try {
            & docker compose --env-file $EnvFile exec -T -e "SQLCMDPASSWORD=$password" sqlserver /opt/mssql-tools18/bin/sqlcmd -S localhost -U sa -C -b -I -d master -Q "IF DB_ID(N'$db') IS NOT NULL BEGIN ALTER DATABASE [$db] SET SINGLE_USER WITH ROLLBACK IMMEDIATE; DROP DATABASE [$db]; END;"
            if ($LASTEXITCODE -ne 0) { Write-Warning "Falha ao remover banco E2E temporario $db." }
        }
        finally { Pop-Location }
    }
}
) { throw "Nome de banco E2E invalido: $db" }
