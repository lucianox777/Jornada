...
active_sehab_person_schema_ref="$(scalar "SELECT gpv.pessoa_schema_ref FROM ref.gestor g JOIN ref.gestor_pessoa_versao gpv ON gpv.gestor_id=g.gestor_id WHERE g.codigo='SEHAB' AND gpv.status='ATIVA';")"
