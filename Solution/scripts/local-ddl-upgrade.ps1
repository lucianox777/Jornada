...
function Sql([string[]]$a){ Push-Location $Root; try { & docker compose --env-file $EnvFile exec -T -e "SQLCMDPASSWORD=$password" sqlserver /opt/mssql-tools18/bin/sqlcmd -S localhost -U sa -C -b @a; if($LASTEXITCODE -ne 0){ throw 'sqlcmd falhou.' } } finally { Pop-Location } }
