...
O modelo físico corrente é `Documentos/Anexo_Modelo_Fisico_Jornada_v1.40`, derivado do schema canônico `Solution/database/Jornada_Fase1_v3.70.sql`. O inventário automatizado predecessor de 12/09/2026 media 69 tabelas. O inventário automatizado do PR #377 (run `35533662679`) mede **78 tabelas distintas** antes da trilha de promoção. A candidata corrente acrescenta `auditoria.modelo_linkage_estado_evento` e passa a **79 tabelas distintas**: 53 do baseline legado, 2 do núcleo progressivo e 24 tabelas distintas introduzidas por migrações. Entre as estruturas fora do baseline estão a referência versionada de frequências de nomes, identidade progressiva/composição, monitor operacional e o ledger append-only de autoria dos atos governados de identidade e a trilha append-only de transições do modelo de Linkage. As antigas contagens de 53, 64, 66, 69 e a estimativa intermediária 70 não devem ser usadas como contagem do schema candidato corrente.
