...
O modelo físico corrente é `Documentos/Anexo_Modelo_Fisico_Jornada_v1.40`, derivado do schema canônico `Solution/database/Jornada_Fase1_v3.70.sql`. O inventário automatizado predecessor de 12/09/2026 media 69 tabelas. A candidata corrente acrescenta `auditoria.decisao_identidade_evento` pelo manifesto canônico e passa a **70 tabelas distintas**: 53 do baseline legado, 2 do núcleo progressivo e 15 tabelas adicionais distintas introduzidas por migrações. Entre as estruturas acrescentadas estão a referência versionada de frequências de nomes e o ledger append-only de autoria dos atos governados de identidade. As antigas contagens de 53, 64, 66 e 69 não devem ser usadas como contagem do schema candidato corrente.
