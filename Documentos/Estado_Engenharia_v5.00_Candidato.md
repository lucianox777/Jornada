...
A verdade de referência dessa etapa vem de observações ligadas deterministicamente por CPF, mas o CPF é ocultado da geração de candidatos e do score. Cada observação rotulada gera um cenário positivo e um cenário negativo `LEAVE_TRUTH_OUT`, no qual a identidade verdadeira é removida do ranking. A calibração usa orçamento FP de engenharia previamente declarado (defaults operacionais de 100 bp para VALIDATION e TEST), minimizando FN entre candidatos admitidos. A contagem FP inclui pessoa errada em positivos e associação indevida no negativo leave-truth-out. O orçamento usa **positivos como denominador**, sem diluir a taxa pelo negativo leave-truth-out; o total de cenários também é persistido. O teto discreto e sua taxa efetiva são auditados; 1% nominal no corpus sintético não demonstra 1% populacional. `TEST` pode reprovar o candidato congelado, nunca escolher outro threshold olhando o próprio teste.

A implementação operacional dessa etapa compartilha literalmente o scorer e a política de decisão com o Runner por `Jornada.Linkage.Core`, evitando uma segunda implementação da fronteira. O draft persiste proveniência, tamanhos das partições e FP/FN/inconclusivos de validação/teste, e `VALIDATE` e `ACTIVATE` falham fechado sem evidência completa da calibração, se FP superar o teto congelado de qualquer partição ou se a conferência governada não estiver CONFORME.
...
A tolerância técnica de engenharia `V1_2026-09-26` foi declarada ex ante e congelada em `0,01` de diferença absoluta de LLR por par; a decisão operacional final deve permanecer exatamente equivalente. Isso compara C# decimal com a implementação independente C# float64, não valida comparadores nem constitui medição Python/Splink. Resta caracterização numérica independente abrangente; fixtures `TEST_ONLY_NOT_GOVERNANCE` não substituem o contrato versionado.
...
O wiring do assert em `VALIDATE` e `ACTIVATE` **já está implementado**: ambas as operações compartilham `LoadPromotionConferenceTolerance`/`AddConferenceGateParameters` e verificam a evidência governada mais recente/fingerprint por `auditoria.sp_assert_conferencia_linkage_conforme` na própria transação `SERIALIZABLE`. A configuração técnica agora contém `toleranceVersion=V1_2026-09-26` e `0,01`; o bloqueio por indefinição numérica foi removido, mas a primeira conferência real por modelo DEV, a caracterização numérica e a avaliação estatística #31 ainda não foram demonstradas. `VALIDATE` e `ACTIVATE` permanecem fail-closed sem evidência `CONFORME` mais recente e fingerprint inalterado. Essa integração não substitui a validação estatística representativa #31.
...
A confirmação humana deixa de depender apenas de texto livre e possui somente duas formas estruturadas: `DOCUMENTO_VERIFICADO` (com `documento_tipo_codigo`) ou `CONFIRMACAO_INSTITUCIONAL_SEM_DOCUMENTO`. Eventos que não constituem confirmação usam `evidencia_tipo = NULL`; abertura e aplicação continuam diferenciadas por `evento_tipo`.
